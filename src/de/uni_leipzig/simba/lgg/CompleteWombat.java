 ...
 * @author sherif
 */
public class CompleteWombat implements Wombat {
	...
	public static int MAX_ITER_NR = 3;//Integer.MAX_VALUE;
	public static int MAX_ITER_TIME_MIN = 10;
...
//				&& root.size() <= MAX_TREE_SIZE
	...
	private ExtendedClassifier getInitialClassifier(String sourceProperty, String targetProperty, String measure) {
		...
		for (double threshold = 1d; threshold > MIN_THRESHOLD; threshold = threshold * learningRate) {
			...
			double overlap = PRFCalculator.fScore(mapping, reference);
...
//			}
	...
	private List<RefinementNode> refine(final Tree<RefinementNode> node) {
		...
		if(isRoot(nodeMetricExpr)){
			...
			for(int i = 0 ; i < subMetricExpr.size() ; i++){
				...
				childMap = SetOperations.intersection(getMapingOfMetricExpression(childSubMetricExpr.get(0)), getMapingOfMetricExpression(childSubMetricExpr.get(1)));
				for(int k = 2 ; k <childSubMetricExpr.size();  k++){
					childMetricExpr = "AND(" + childMetricExpr + "," + childSubMetricExpr.get(k) + ")|0.0";
					childMap = SetOperations.intersection(childMap, getMapingOfMetricExpression(childSubMetricExpr.get(k)));
				}
			...
			for(int i = 0 ; i < subMetricExpr.size() ; i++){
				...
				childMap = SetOperations.union(getMapingOfMetricExpression(childSubMetricExpr.get(0)), getMapingOfMetricExpression(childSubMetricExpr.get(1)));
				for(int k = 2 ; k <childSubMetricExpr.size();  k++){
					childMetricExpr = "OR(" + childMetricExpr + "," + childSubMetricExpr.get(k) + ")|0.0";
					childMap = SetOperations.union(childMap, getMapingOfMetricExpression(childSubMetricExpr.get(k)));
				}
				result.add(createNode(childMetricExpr, childMap));
				childMetricExpr = new String();
			}
	 ...
	 * @author sherif
	 */
	private Mapping getMapingOfMetricExpression(String metricExpression) {
		Mapping map = null;
		if(RefinementNode.saveMapping){
...
//		t.remove();
	...
	public static void main(String args[]) {
		...
		CompleteWombat clgg = new CompleteWombat(source, target, reference, 0.6);
