package  de.uni_leipzig.simba.transferlearning.transfer;
...
import de.uni_leipzig.simba.transferlearning.replacement.Replacer;

import de.uni_leipzig.simba.transferlearning.transfer.classes.ClassSimilarity;
import de.uni_leipzig.simba.transferlearning.transfer.classes.SamplingBasedClassSimilarity;
import de.uni_leipzig.simba.transferlearning.transfer.classes.UriBasedClassSimilarity;
import de.uni_leipzig.simba.transferlearning.transfer.config.ConfigAccuracy;
import de.uni_leipzig.simba.transferlearning.transfer.config.ConfigAccuracyWald95;
...
import de.uni_leipzig.simba.transferlearning.transfer.properties.UriBasedPropertySimilarity;
import de.uni_leipzig.simba.transferlearning.transfer.properties.PropertySimilarity;
...
public class TransferLearner {
    ...
    public Map<Configuration, Double> runSimpleMatching(int instanceSampleSize, double coverage) {
        ...
        for (Configuration config : configurations) {
            ...
            Mapping reference = new Mapping().readNtFile(config.name.replaceAll(Pattern.quote("spec.xml"), "accept.nt")); //accept file represents he gold standard
    ...
    public Configuration runLeaveOneOut(Configuration config, String inputSourceClass, String inputTargetClass, Set<String> relevantSourceProperties, Set<String> relevantTargetProperties) {
        ...
        for (Configuration configuration : configurations) {
            ...
            if (!name1.equalsIgnoreCase(name2)) {//not to b compared to itself
                ...
                double cSSim = classSim.getSimilarity(sourceClass, inputSourceClass, config, configuration,isSource);
                double cTSim = classSim.getSimilarity(targetClass, inputTargetClass, config, configuration,!(isSource));
...
//		String inputTargetClass = "http://www4.wiwiss.fu-berlin.de/diseasome/resource/diseasome/diseases";
    ...
    /**
     * returns list of similar configurations in terms of class similarity sorted by similarity degree descending
            ...
            if (!name1.equalsIgnoreCase(name2)) {
                ...
                for (String sourceClass : sourceClasses) {
                    ...
                    for (String inputSourceClass : inputSourceClasses) {
                    	...
                    	sim = ((SamplingBasedClassSimilarity)classSim).getSimilarity(sourceClass, inputSourceClass, config,configuration,isSource);
                        if (sim > max) {
                            max = sim;
                        }
                    }
                ...
                for (String targetClass : targetClasses) {
                    ...
                    for (String inputTargetClass : inputTargetClasses) {
                        sim = classSim.getSimilarity(targetClass, inputTargetClass, config, configuration,!isSource);
                        if (sim > max) {
                            max = sim;
                        }
                    }
