...
public class Eval {
	...
	private File specsDir = new File("finalSpecs/"/*"specs/"*/);
	private File usableSpecDirsFile = new File(specsDir, "usableSpecDirs.txt");
	private List<String> usableSpecs;
	private String limesSpecName = "spec.xml";//"spec.limes.xml";
	...
	public List<String> getUsableSpecDirs() {
		...
		Collection<File> specDirs = FileUtils.listFiles(specsDir, DirectoryFileFilter.INSTANCE, DirectoryFileFilter.INSTANCE);
		dsipayDirs(specDirs);
		...
		for(File specDir : specDirs) {
			...
			//System.out.println(limesSpec.getName());
	...
	public void dsipayDirs(Collection<File> specDirs)
	{
		for(File specDir : specDirs){
