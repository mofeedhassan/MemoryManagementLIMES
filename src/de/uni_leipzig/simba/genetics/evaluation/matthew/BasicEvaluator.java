...
//			pop.addFittestProgram((IGPProgram) pBest.clone());
		...
		EvaluationPseudoMemory mem = new EvaluationPseudoMemory(run, gen, getMetric(pBest), fittest, 1d-fittest, clock.totalDuration());
	...
	public void finishRun() {
		...
		double crossProduct = fitness.getSourceCache().size()*fitness.getTargetCache().size();
		for(EvaluationPseudoMemory mem : perRunMemory) {
			...
			mem.matthew = pRF.computeMatthewsCorrelation(fullMap, o.getMapping(), crossProduct);
