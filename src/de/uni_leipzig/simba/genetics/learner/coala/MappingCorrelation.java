...
public class MappingCorrelation {
	 ...
	 */
	protected	 HashMap<Pair<String>,StringMeasure> propMeasureMap ;
	private static final String CLUSTER_FILE = "cluster.txt";
	public MappingCorrelation(KBInfo source, KBInfo target, String metric, Cache sC, Cache tC){
		this(source,target,metric,DEFAULT_MAX_PROP, sC, tC);
	}
	
	public MappingCorrelation (KBInfo source, KBInfo target, String metric, int maxProperties, Cache sC, Cache tC){
		this.source = source;
		this.target = target;
		this.metric = metric;
		...
		sourceCache = sC;//HybridCache.getData(source);
		targetCache = tC;//HybridCache.getData(target);
	}
	 ...
	 * @return
	 */
	protected TreeMap<Float,List<Triple>> initInformativeTripleMap(List <Triple> tripleList){
	...
	protected double calculateDistance(Triple t1, Triple t2,
...
//			System.out.println("calcDist"+pair+" \n t1:"+t1+" \n t2:"+t2+"\n"+similarityCache);
	 ...
	 */
	protected HashMap<Integer,HashMap<String,Double>> initSimilarityCache (List <Triple> triples) {
	...
	private List <Triple> initOracleList(List<Triple> initList, int trainingDataSize, HashMap<String, Integer> reverseMap,
			...
			for (int tripleId =0;tripleId<initList.size();tripleId++){ //
				...
				if (!visitPosCluster.contains(reverseMap.get(t.hashCode()+""))&&
					...
					// only chosen if a other triple was not chosen in the same cluster
	 ...
	 * @return
	 */
	protected HashMap <Pair<String>,StringMeasure> getStringMeasures(String metric){
		String copy;
