...
import de.uni_leipzig.simba.genetics.commands.AtomicPreprocessingCommand;
import de.uni_leipzig.simba.genetics.commands.ChainedPreprocessingCommand;
import de.uni_leipzig.simba.genetics.commands.DateMeasure;
import de.uni_leipzig.simba.genetics.commands.DatePropertyPair;
...
import de.uni_leipzig.simba.genetics.commands.PointSetMeasure;
import de.uni_leipzig.simba.genetics.commands.PointSetPropertyPair;
import de.uni_leipzig.simba.genetics.commands.StringMeasure;
import de.uni_leipzig.simba.genetics.commands.StringPreprocessMeasure;
	...
	public GPGenotype create() throws InvalidConfigurationException {
		...
		LinkSpecGeneticLearnerConfig config = (LinkSpecGeneticLearnerConfig) getGPConfiguration();
			//	ExpressionApplicationData applData = new ExpressionApplicationData("PublicationData.xml");
				...
				SUBPROGRAM = new SubProgram(config, new Class[] {String.class, String.class, CommandGene.IntegerClass}, true);
...
//				System.out.println("Nodes..."+nodes);
	...
	private List<CommandGene> getBasicNodes(LinkSpecGeneticLearnerConfig config) throws InvalidConfigurationException {
		...
		List<CommandGene> nodes = new LinkedList<CommandGene>();
			...
			nodes.add(new Terminal(config, CommandGene.IntegerClass, 9, 100, false, 
...
//				nodes.add( new StringPropertyPair(config, Pair.class, ResourceTerminalType.STRINGPROPPAIR.intValue(), true, i));
		    ...
		    nodes.add(new Terminal(config, CommandGene.IntegerClass, 80, 100, false, 
...
//		}
		...
		if(config.redundantCommands()) {
			nodes.add( new Terminal(config, CommandGene.IntegerClass, 9, 100, true,
		    		ResourceTerminalType.GOBALTHRESHOLD.intValue(), false));
