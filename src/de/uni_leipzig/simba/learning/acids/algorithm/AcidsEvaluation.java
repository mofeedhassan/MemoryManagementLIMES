...
public class AcidsEvaluation {
	
	private SvmHandler svmHandler;
	private AcidsSetting setting;

	public AcidsEvaluation(AcidsSetting setting) {
		super();
	...
	public Fscore fastEvaluation(ArrayList<Resource> sources, ArrayList<Resource> targets) {
		...
		ArrayList<AcidsSimilarity> sims = setting.getMeasures().getAllSimilarities();
		...
		for(int index=0; index<sims.size(); index++) {
			AcidsSimilarity sim = sims.get(index);
			...
			if(sim.isComputed() && sim.getFilter() != null) {
				...
				if(performCartesianP) { // first property works on the entire Cartesian product.
					if(sim.getProperty().getDatatype() == AcidsDatatype.TYPE_STRING)
...
//		TODO Implement "keep best match only".
//		HashMap<Resource, Double> scores = new HashMap<Resource, Double>();
		
		for(Resource s : sources) {
			for(Resource t : targets) {
				for(int j=0; j<sims.size(); j++) {
					AcidsSimilarity sim = sims.get(j);
