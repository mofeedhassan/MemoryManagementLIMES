...
public class BooleanClassifierLearner implements Learner {
     ...
     */
    public BooleanClassifierLearner(KBInfo sInfo, KBInfo tInfo, Cache s, Cache t, Oracle o,
        ...
        config = new BooleanClassifier(propertyMapping, propertyType);
        //getLinks for given config;
        mapper = SetConstraintsMapperFactory.getMapper("simple", targetInfo,
                sourceInfo, target, source, new LinearFilter(), GRANULARITY);
        //cr.source, cr.target, source, target, new PPJoinMapper(), new LinearFilter());

        double threshold = config.INIT_THRESHOLD * REGRESSION;
        logger.info("Getting initial links with threshold " + threshold);
        results = new Mapping();
        while (results.size < 10) {

            results = mapper.getLinks(config.getExpression(REGRESSION), 0);
    ...
    public boolean computeNextConfig(int n) {
        ...
        Mapping mostInfPos = getMostInformativePositives(results, config.INIT_THRESHOLD, positives, n / 2);
        Mapping mostInfNeg = getMostInformativeNegatives(results, config.INIT_THRESHOLD, negatives, n / 2);
...
//                }
            ...
            oldResults = results;
            results = mapper.getLinks(config.getExpression(), 0);
            logger.info(getPRF());
        } else {
    ...
    public Mapping returnFinalResults() {
        return mapper.getLinks(config.getExpression(), 0);
    }
