...
//		System.out.println(start+nodesTags+edgeTags+end);
	...
	private boolean equality(Object first,Object second) {
		LinkSpec f = (LinkSpec) first;
    	LinkSpec s = (LinkSpec) second;
		int i=0;
		if(f.threshold == 0.9025 && s.threshold == 0.95)
			i=3;
    	List<LinkSpec> children = new ArrayList<LinkSpec>();
    		if(f.isAtomic() && s.isAtomic()) {
    			//mofeed:start
    			if(f.getFilterExpression() == null && s.getFilterExpression() == null)
    				return Math.abs(f.threshold-s.threshold) < 0.02d;
    			...
    			//mofeed:end
    				...
    				return Math.abs(f.threshold-s.threshold) < 0.01d; //that means similar in filter expression and threshold too
    			...
    			if(f.operator == null && s.operator == null)
    				return true;
    			else if((f.operator != null && s.operator == null) || (f.operator == null && s.operator != null))
    				return false;
    			if(f.operator.equals(s.operator)) {
//    				if(this.children.size()==o.children.size()) {
    					HashSet<LinkSpec> hs = new HashSet<LinkSpec>();
    					hs.addAll(children);
    					boolean b = hs.addAll(s.children);
    					return(hs.addAll(s.children));//true if this collection changed as a result of the call
    			}
