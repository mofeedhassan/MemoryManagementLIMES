 ...
 * @author Axel
 */
public class DensityBasedOnePassOrganizer extends DensityBasedOrganizer {

    public double similarityThreshold = 1.0f;
        ...
        for (int i = 1; i < uris.size(); i++) {
            ...
            for (int j = 0; j < exemplarList.size(); j++) {
                value = m.getSimilarity(exemplarList.get(j), c.getInstance(uris.get(i)))/
                        java.lang.Math.sqrt(1+exemplarMap.get(exemplarList.get(j)).size());
                comparisons++;
                if (value >= max) {
                    max = value;
                    index = j;
